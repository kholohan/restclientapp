...
public class Mapper
{
	public static void mapFreeGeoToLocationWeatherInfo(FreeGeoResult result,
			LocationWeatherInfo locationWeatherInfo)
	{
		...
		locationInfo.setRegion(result.getRegionCode());
	...
	public static void mapZippopotamusToLocationWeatherInfo(
			ZippopotamusResult result, LocationWeatherInfo locationWeatherInfo)
	{
		// null check
		locationWeatherInfo.setCountry(result.getCountryCode());
		...
		for (Place place : result.getPlaces())
		{
			...
			locationInfo.setRegion(place.getRegionCode());
	...
	public static void mapOpenWeatherToLocationInfo(
			OpenWeatherResult openWeatherResult, LocationInfo locationInfo)
	{
		// null check
		Temperature temperature = openWeatherResult.getTemperature();
		Double temperatureKelvin = temperature.getTemperature();
		Double temperatureFarenheit = TemperatureUtil
				.kelvinToFarenheit(temperatureKelvin);
